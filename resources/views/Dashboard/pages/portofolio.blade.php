            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse justify-content-end" id="navigation">
                    <ul class="nav navbar-nav mr-auto">
                        ...
                        <li class="nav-item">
                            <a class="nav-link">
                                <form method="GET" action="{{ url('/cms/portofolio') }}" class="form-search">
                                    <label for="search">
                    ...
                    <main class="d-flex flex-column gap-3 grow">
                        <section class="d-flex gap-2 items-center justify-content-between">
                            <div class="d-flex align-items-center ml-auto">
                                <a href="{{ url('/cms/addportofolio')}}" type="button"
                                ...
                                <div class="card-body px-0 pt-0 pb-2">
                                    <div class="table-responsive p-0">
                                        <table class="table align-items-center mb-0 table-hover">
                                            ...
                                            <tbody>
                                                @foreach ($data['get_data'] as $portofolio)
                                                <tr>
                                                    <td class="text-center">{{ $nomor++ }}</td>
                                                    <td class="text-center"><img style="max-height: 100px"
                                                            src="{{ asset('storage/' . $portofolio->thumbnail_portofolio) }}"
                                                            alt=""></td>
                                                    <td class="text-center">{{ $portofolio->title_portofolio }}</td>
                                                    <td class="text-center">{{ $portofolio->description }}</td>
                                                    {{-- <td class="text-center">
                                                        <a href="{{ $portofolio->link_portofolio }}" target="_blank"
                                                    >{{ $portofolio->link_portofolio }}</a>
                                                    </td> --}}
                                                    <td class="text-center">
                                                        <a href="{{ $portofolio->link_portofolio }}"
                                                            target="_blank">{{ strlen($portofolio->link_portofolio) > 20 ? substr($portofolio->link_portofolio, 0, 20) . '...' : $portofolio->link_portofolio }}</a>
                                                    </td>

                                                    <td>
                                                        <div class="d-flex">
                                                            <a href="/cms/{{ $portofolio->id }}/editportofolio"
                                                                class="btn btn-primary btn-sm bg-primary me-1 text-white">Edit</a>
                                                            <form action="/cms/portofolio/{{ $portofolio->id }}"
                                                                method="POST"
