    ...
    <div class="main-panel">
        ...
        <nav class="navbar navbar-expand-lg " color-on-scroll="500">
            <div class="container-fluid">
                <a class="navbar-brand"> Struktur Kantor Wilayah {{ $card['name_kantorwilayah']}} </a>
                ...
                <div class="collapse navbar-collapse justify-content-end" id="navigation">
                    <ul class="nav navbar-nav mr-auto">
                        ...
                        <li class="nav-item">
                            <a class="nav-link">
                                <form method="GET" action="/cms/{{ $card['get_kanwil']->id }}/struktur_kantorwilayah" class="form-search">
                                    <label for="search">
                                        <input required="" name="keyword" autocomplete="off" placeholder="search name here"
                    ...
                    <main class="d-flex flex-column gap-3 grow">
                        <section class="d-flex gap-2 items-center justify-content-between">
                            <div class="d-flex align-items-center ml-auto">
                                <a href="{{ url('/cms/' . $card['get_kanwil']->id . '/struktur_kanwil/addstruktur') }}" type="button" class="btn btn-primary rounded-lg">
                                ...
                                <div class="card-body px-0 pt-0 pb-2">
                                    <div class="table-responsive p-0">
                                        <table class="table align-items-center mb-0 table-hover">
                                            ...
                                            <tbody>
                                                @foreach ($card['get_data'] as $struktur)
                                                <tr>
                                                    <td class="text-center">{{ $nomor++ }}</td>
                                                    <td class="text-center"><img style="max-height: 100px"
                                                        src="{{ asset('storage/' . $struktur->thumbnail_struktur) }}" alt=""></td>
                                                    <td class="text-center">{{ $struktur->name_struktur }}</td>
                                                    <td class="text-center">{{ $struktur->position->name }}</td>
                                                    <td>
                                                        <div class="d-flex">
                                                            <a href="{{ url('/cms/' . $card['get_kanwil']->id . '/struktur_kanwil/' . $struktur->id . '/editstruktur') }}"
                                                                class="btn btn-primary btn-sm bg-primary me-1 text-white">Edit</a>
                                                            <form
                                                                action="{{ url('/cms/' . $card['get_kanwil']->id . '/struktur_kanwil/'. $struktur->id) }}"
                                                                method="POST"
