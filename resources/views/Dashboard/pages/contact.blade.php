            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse justify-content-end" id="navigation">
                    <ul class="nav navbar-nav mr-auto">
                        ...
                        <li class="nav-item">
                            <a class="nav-link">
                                <form method="GET" action="{{ url('/cms/contact') }}" class="form-search">
                                    <label for="search">
                                        <input required="" autocomplete="off" name="keyword" placeholder="search your content" id="search" type="text" value="{{ $data['keyword'] }}">
                                        <div class="icon">
                    ...
                    <main class="d-flex flex-column gap-3 grow">
                        <section class="d-flex gap-2 items-center justify-content-between">
                            <div class="d-flex align-items-center ml-auto">
                                <a href="{{ url('/cms/addcontact')}}" type="button"
                                        ...
                                        <table class="table align-items-center mb-0 table-hover">
                                            <thead class="bg-grey1">
                                                <tr>
                                                    ...
                                                    <th class="text-center">Project Office</th>
                                                    <th class="text-center">Detail Project</th>
                                                    <th class="text-center">Deadline</th>
                                                    <th class="text-center">Status</th>
                                                    <th>Actions</th>
                                                </tr>
                                            <tbody>
                                                @foreach ($data['get_data'] as $contact)
                                                <tr>
                                                    <td class="text-center">{{ $nomor++ }}</td>
                                                    <td class="text-center">{{ $contact->klien }}</td>
                                                    <td class="text-center">{{ $contact->email }}</td>
                                                    <td class="text-center">{{ $contact->telepon }}</td>
                                                    <td class="text-center">{{ $contact->service }}</td>
                                                    <td class="text-center">{{ $contact->kantorwilayah }}</td>
                                                    <td class="text-center">{{ $contact->detail }}</td>
                                                    {{-- <td class="text-center">{{ strlen($contact->detail) > 10 ? substr($contact->detail, 0, 10) . '...' : $contact->detail }}</td> --}}
                                                    <td class="text-center">{{ $contact->created_at_formatted }}</td>
                                                    <td class="text-center">{{ $contact->status }}</td>
                                                    <td>
                                                        <div class="d-flex">
                                                            <a href="/cms/{{ $contact->id }}/editcontact"
                                                                class="btn btn-primary btn-sm bg-primary me-1 text-white">Edit</a>
                                                                <form action="/cms/contact/{{ $contact->id }}" method="POST"
